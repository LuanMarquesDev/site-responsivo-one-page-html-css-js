...
$( document ).ready(function() {
  ...
  let circleB = new ProgressBar.Circle(containerB, {
    ...
    step: function(state, circle) {
      ...
      var value = Math.round(circle.value() * 270);
      circle.setText(value);
  ...
  let dataAreaOffset = $('#circle').offset();
...
$('.filter-btn').on('click', function() {
  ...
  if(type == 'dsg-btn') {
    ...
    eachBoxes('dev', boxes);
  } else if(type == 'project-btn') {
    eachBoxes('project', boxes);
  } else if(type == 'misc-btn') {
      eachBoxes('misc', boxes);
  } else {
    eachBoxes('all', boxes);
  }
